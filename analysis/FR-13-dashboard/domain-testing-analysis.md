...
| EC-AMOUNT-V01 | `order.total_amount` | `total_amount` của đơn `delivered` được cộng vào tổng doanh thu. | Valid | `30000000`, `6000000`, `45000000` | FR-13 | Giá trị cụ thể dùng để quan sát phép cộng. |
| EC-AMOUNT-I01 | `order.total_amount` | `total_amount` của đơn không phải `delivered` không được cộng vào tổng doanh thu. | Invalid | `28000000`, `6000000`, `4000000`, `30000000` | FR-13 | Invalid theo điều kiện được cộng vào doanh thu, không khẳng định số tiền không hợp lệ. |
...
| COND-FR13-DT-002 | EC-SESSION-V01 | EC-DATASET-V02 | 1 đơn `delivered`, `total_amount = 30000000` | Valid | Dashboard hiển thị tổng doanh thu `30000000` (`30,000,000 ₫` nếu UI định dạng tiền tệ) và tổng số đơn hàng `1`. | EC-SESSION-V01, EC-DATASET-V02, EC-STATUS-V01, EC-AMOUNT-V01, EC-COUNT-V02, DC-02, DC-03 | Kiểm tra trường hợp tối thiểu có dữ liệu doanh thu. |
| COND-FR13-DT-003 | EC-SESSION-V01 | EC-DATASET-V03 | 2 đơn `delivered`, `total_amount = 30000000` và `6000000` | Valid | Dashboard hiển thị tổng doanh thu `36000000` (`36,000,000 ₫` nếu UI định dạng tiền tệ) và tổng số đơn hàng `2`. | EC-SESSION-V01, EC-DATASET-V03, EC-STATUS-V01, EC-AMOUNT-V01, EC-COUNT-V02, DC-02, DC-03 | Kiểm tra cộng dồn nhiều đơn delivered. |
| COND-FR13-DT-004 | EC-SESSION-V01 | EC-DATASET-V03 | 1 đơn `delivered = 45000000`; 4 đơn không delivered có tổng `68000000` | Valid | Dashboard hiển thị tổng doanh thu `45000000` (`45,000,000 ₫` nếu UI định dạng tiền tệ) và tổng số đơn hàng `5`. | EC-SESSION-V01, EC-DATASET-V03, EC-STATUS-V01, EC-STATUS-I01, EC-AMOUNT-V01, EC-AMOUNT-I01, EC-COUNT-V02, DC-02, DC-03 | Kiểm tra loại trừ doanh thu của trạng thái không delivered nhưng vẫn đếm đơn. |
| COND-FR13-DT-005 | EC-SESSION-V01 | EC-DATASET-V03 | 4 đơn không delivered, không có đơn `delivered`, mỗi đơn có `total_amount = 30000000` | Valid | Dashboard hiển thị tổng doanh thu `0` và tổng số đơn hàng `4`. | EC-SESSION-V01, EC-DATASET-V03, EC-STATUS-I01, EC-AMOUNT-I01, EC-COUNT-V02, DC-02, DC-03 | Kiểm tra không có đơn nào đủ điều kiện doanh thu. |
