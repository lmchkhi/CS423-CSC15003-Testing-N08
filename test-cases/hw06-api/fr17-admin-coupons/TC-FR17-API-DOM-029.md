# TC-FR17-API-DOM-029: `discount_value` là null
...
| Request body/query/path | `{"code":"HW06DISCNULL{{runId}}","type":"fixed","discount_value":null,"min_order_amount":0,"expired_at":"2099-12-31","max_uses_per_user":1}` |
...
- HTTP status: `400`.
- Response body/headers/assertions: message hoặc error; không tạo coupon.
- Không có unexpected `5xx` nếu đây là negative/security case.
- Rationale: Null partition cho required positive number.
...
| Data row key | `TC-FR17-API-DOM-029` |
...
| Human reasoning | Human thêm để hoàn thiện null partition cho `discount_value`. |
| Why AI missed it | AI có missing/zero/negative/string nhưng bỏ sót null discount_value. |
