# TC-FR17-API-SEC-004: Normal user token không thể create admin coupon
...
- Login bằng user thường `test@eshop.com`.
...
| Authorization | `user token / invalid auth as case states` |
| Request body/query/path | Valid coupon body với `Authorization: Bearer {{userToken}}` |
...
- Rationale: SEC-03 yêu cầu role admin, không chỉ token tồn tại.
...
| Data row key | `TC-FR17-API-SEC-004` |
...
Related bug report: `reports/bug-reports/BUG-FR17-API-003.md`
