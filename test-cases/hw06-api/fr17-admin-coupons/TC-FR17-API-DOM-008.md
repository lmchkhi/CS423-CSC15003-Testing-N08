# TC-FR17-API-DOM-008: Duplicate `code` bị reject
...
- Admin đã tạo coupon với code `HW06DUP{{runId}}` trong setup.
...
| Request body/query/path | Request create thứ hai dùng cùng `code` và body hợp lệ |
...
- Response body/headers/assertions: message hoặc error; list verify chỉ có một coupon với code đó.
- Không có unexpected `5xx` nếu đây là negative/security case.
- Rationale: FR-17 yêu cầu `code` duy nhất.
...
| Data row key | `TC-FR17-API-DOM-008` |
...
| Human reasoning | Ý tưởng đúng nhưng raw thiếu setup/list verify; sửa thành sequence create trước rồi duplicate create và verify không nhân bản. |
