# TC-FR17-API-WF-008: Deleted coupon code có thể được tạo lại sau cleanup
...
- Admin tạo coupon `HW06RECREATE{{runId}}`, delete thành công.
...
| Request body/query/path | Create -> DELETE -> create lại cùng code |
...
- HTTP status: `200/201 then 200/204 then 200/201`.
- Response body/headers/assertions: Second create succeeds; final cleanup bằng DELETE.
- Không có unexpected `5xx` nếu đây là negative/security case.
- Rationale: Kiểm tra uniqueness lifecycle sau cleanup.
...
| Data row key | `TC-FR17-API-WF-008` |
...
| Human reasoning | Human thêm để xác nhận cleanup không khóa vĩnh viễn unique code. |
| Why AI missed it | AI có delete/list verify nhưng bỏ sót recreate-after-delete lifecycle. |
