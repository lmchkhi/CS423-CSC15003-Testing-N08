# TC-FR17-API-WF-005: Failed validation create không tạo partial coupon
...
- Admin logged in; unique invalid code.
...
| Request body/query/path | Invalid create thiếu `discount_value`, sau đó `GET /api/coupons` |
...
- Response body/headers/assertions: Invalid code absent from list.
- Không có unexpected `5xx` nếu đây là negative/security case.
- Rationale: Failed create không được để lại partial state.
...
| Data row key | `TC-FR17-API-WF-005` |
