# TC-FR17-API-WF-003: Created coupon có thể cleanup bằng DELETE
...
- Admin created coupon và capture `{{createdCouponId}}`.
...
| Request body/query/path | `DELETE /api/admin/coupons/{{createdCouponId}}` |
...
- Response body/headers/assertions: Delete success; không 5xx.
- Không có unexpected `5xx` nếu đây là negative/security case.
- Rationale: Phase 10 cần cleanup cho stateful create tests.
...
| Data row key | `TC-FR17-API-WF-003` |
...
| Human reasoning | Raw dùng API phụ DELETE; giữ làm cleanup strategy, exact status chấp nhận 200/204 vì spec chưa nêu. |
