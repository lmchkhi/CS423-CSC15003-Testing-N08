# TC-FR17-API-SEC-007: XSS-like payload trong `code` không gây 500 hoặc unsafe reflection
...
| Request body/query/path | `{"code":"<script>alert(1)</script>","type":"fixed","discount_value":10000,"min_order_amount":0,"expired_at":"2099-12-31","max_uses_per_user":1}` |
...
- HTTP status: `400 hoặc 200/201`.
- Response body/headers/assertions: Không 5xx; không reflect executable script; nếu được tạo thì phải cleanup và list response phải an toàn.
- Không có unexpected `5xx` nếu đây là negative/security case.
- Rationale: SEC-04/SEC-05: input UI-visible phải xử lý an toàn.
...
| Data row key | `TC-FR17-API-SEC-007` |
...
| Human reasoning | Raw đúng ý tưởng security nhưng oracle phụ thuộc việc API reject hay lưu an toàn; giữ assertion trọng tâm no 5xx/no unsafe reflection. |
