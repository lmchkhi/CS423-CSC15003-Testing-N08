# TC-FR17-API-DOM-023: `expired_at` trong quá khứ vẫn có thể được admin tạo như coupon expired
...
- Admin logged in; unique code `HW06PAST{{runId}}`.
...
| Request body/query/path | `{"code":"HW06PAST{{runId}}","type":"percent","discount_value":10,"min_order_amount":0,"expired_at":"2020-01-01","max_uses_per_user":1}` |
...
- Response body/headers/assertions: JSON success; coupon nếu tạo phải có expired_at đúng và cần cleanup.
- Không có unexpected `5xx` nếu đây là negative/security case.
- Rationale: FR-17 không cấm tạo coupon đã hết hạn; hệ thống đã có sample `EXPIRED`.
...
| Data row key | `TC-FR17-API-DOM-023` |
...
| Human reasoning | Raw expected mơ hồ; sửa theo blackbox requirement: CRUD create cho phép expiration date bắt buộc, không bắt buộc future date. |
