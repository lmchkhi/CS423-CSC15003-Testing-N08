# TC-FR17-API-SCH-001: Success response có Content-Type JSON khi có body
...
- Response body/headers/assertions: Nếu response có body thì header `Content-Type` chứa `application/json`.
- Không có unexpected `5xx` nếu đây là negative/security case.
- Rationale: Create response nên là JSON nếu trả body.
...
| Data row key | `TC-FR17-API-SCH-001` |
...
| Human reasoning | Raw đúng nhưng success response body chưa được spec nêu; sửa assertion có điều kiện nếu có body. |
