# TC-FR17-API-SEC-009: Success/error response không leak token/password fields
...
- Admin logged in; valid coupon body.
...
- Response body/headers/assertions: Không có `password`, `password_hash`, `token`, `jwt` hoặc credential fields.
- Không có unexpected `5xx` nếu đây là negative/security case.
- Rationale: Coupon create response không được expose thông tin nhạy cảm.
...
| Data row key | `TC-FR17-API-SEC-009` |
