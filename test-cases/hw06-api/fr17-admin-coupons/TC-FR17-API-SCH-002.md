# TC-FR17-API-SCH-002: Success response có identity hoặc message ổn định để cleanup

## Requirement ID
FR-17

## API / Test type / Technique
`POST /api/admin/coupons` / API Testing / `Schema Validation`
...
- Admin logged in; valid create body.
...
| Request body/query/path | Valid coupon body |
...
- Response body/headers/assertions: Có `id`/`coupon_id` hoặc `message`; nếu có id thì capture để cleanup; không body rỗng trừ khi spec/observed cho phép.
- Không có unexpected `5xx` nếu đây là negative/security case.
- Rationale: Cần map cleanup/evidence trong Newman.
...
| Data row key | `TC-FR17-API-SCH-002` |
...
| Human reasoning | Raw thiếu exact response shape do api_spec không nêu; giữ assertion linh hoạt và capture id nếu có. |
