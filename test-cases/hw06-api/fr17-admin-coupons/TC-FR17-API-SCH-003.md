# TC-FR17-API-SCH-003: Error response cho missing required field có shape an toàn
...
| Request body/query/path | Missing `code` body |
...
- HTTP status: `400`.
- Response body/headers/assertions: JSON body có `message` hoặc `error` string; không stack/sql trace.
- Không có unexpected `5xx` nếu đây là negative/security case.
- Rationale: Negative schema contract cho validation errors.
...
| Data row key | `TC-FR17-API-SCH-003` |
