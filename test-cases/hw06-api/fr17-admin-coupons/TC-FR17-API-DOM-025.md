# TC-FR17-API-DOM-025: `max_uses_per_user` bằng 1 boundary hợp lệ
...
- Admin logged in; unique code `HW06MAX1{{runId}}`.
...
| Request body/query/path | `{"code":"HW06MAX1{{runId}}","type":"fixed","discount_value":10000,"min_order_amount":0,"expired_at":"2099-12-31","max_uses_per_user":1}` |
...
- Response body/headers/assertions: JSON success; created coupon cần cleanup.
- Không có unexpected `5xx` nếu đây là negative/security case.
- Rationale: FR-17 cho phép `max_uses_per_user >= 1`; 1 là lower boundary hợp lệ.
...
| Data row key | `TC-FR17-API-DOM-025` |
