# TC-FR17-API-WF-006: User token create fail không pollute coupon list
...
- Login as normal user; unique code.
...
| Request body/query/path | `POST /api/admin/coupons` bằng user token, sau đó admin `GET /api/coupons` |
...
- HTTP status: `403 then 200`.
- Response body/headers/assertions: Code absent from list.
- Không có unexpected `5xx` nếu đây là negative/security case.
- Rationale: SEC-03 failure không được mutate state.
...
| Data row key | `TC-FR17-API-WF-006` |
