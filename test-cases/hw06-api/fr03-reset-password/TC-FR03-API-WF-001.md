# TC-FR03-API-WF-001: Full workflow forgot-password -> reset-password
...
- Registered user tồn tại hoặc được tạo bằng register API.
...
| Authorization | `none` |
| Request body/query/path | `1) forgot-password email; 2) reset-password với returned token` |
...
- HTTP status: `200 then 200`.
- Response body/headers/assertions: Setup có `resetToken`; reset có `message`..
...
| Data row key | `TC-FR03-API-WF-001` |
...
| Source | `AI` |
| Audit label | `VALID` |
| Human reasoning | Workflow chính FR-03. |
