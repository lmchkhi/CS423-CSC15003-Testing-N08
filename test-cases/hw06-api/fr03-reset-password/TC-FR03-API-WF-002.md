# TC-FR03-API-WF-002: Reset without prior forgot-password request
...
- Không gọi setup trong test run hiện tại.
...
| Authorization | `none` |
| Request body/query/path | `{"email":"test@eshop.com","resetToken":"123456","newPassword":"NewPass123!"}` |
...
- HTTP status: `400`.
- Response body/headers/assertions: Có `message` hoặc `error`; không 5xx..
...
| Data row key | `TC-FR03-API-WF-002` |
...
| Source | `AI` |
| Audit label | `VALID` |
| Human reasoning | Token phải được issue trước. |
