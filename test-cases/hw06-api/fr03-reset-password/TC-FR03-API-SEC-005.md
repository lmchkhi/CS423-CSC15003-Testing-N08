# TC-FR03-API-SEC-005: Token của email A không reset được email B
...
- Tạo/đảm bảo account A và account B; lấy token cho A.
...
| Authorization | `none` |
| Request body/query/path | `{"email":"{{fr03EmailB}}","resetToken":"{{tokenForEmailA}}","newPassword":"NewPass123!"}` |
...
- HTTP status: `4xx`.
- Response body/headers/assertions: Không reset email B; có `message` hoặc `error`; không 5xx..
...
| Data row key | `TC-FR03-API-SEC-005` |
...
| Human reasoning | Bổ sung setup email B để case execute được. |
