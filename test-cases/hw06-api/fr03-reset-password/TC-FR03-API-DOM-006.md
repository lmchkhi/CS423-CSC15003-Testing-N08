# TC-FR03-API-DOM-006: Unknown/unregistered email không được reset password
...
- Không có account với `unknown-{{runId}}@eshop.test`.
...
| Authorization | `none` |
| Request body/query/path | `{"email":"unknown-{{runId}}@eshop.test","resetToken":"123456","newPassword":"NewPass123!"}` |
...
- HTTP status: `4xx`.
- Response body/headers/assertions: Response không thành công; có `message` hoặc `error`; không trả `resetToken`/password..
...
| Data row key | `TC-FR03-API-DOM-006` |
...
| Human reasoning | Sửa expected 400/404 thành client error 4xx vì spec không định nghĩa exact error status. |
