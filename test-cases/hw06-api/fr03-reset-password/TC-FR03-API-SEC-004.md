# TC-FR03-API-SEC-004: XSS-like payload trong `newPassword` không bị reflect/leak
...
| Authorization | `none` |
| Request body/query/path | `{"email":"test@eshop.com","resetToken":"{{validResetToken}}","newPassword":"<script>alert(1)</script>Aa1!"}` |
...
- HTTP status: `400`.
- Response body/headers/assertions: Không reflect `newPassword`; không 5xx..
...
| Data row key | `TC-FR03-API-SEC-004` |
...
| Source | `AI` |
| Audit label | `VALID` |
| Human reasoning | Password input là sensitive data. |
