# TC-FR03-API-SEC-003: XSS-like payload trong `email` bị xử lý an toàn
...
| Authorization | `none` |
| Request body/query/path | `{"email":"<script>alert(1)</script>@x.test","resetToken":"123456","newPassword":"NewPass123!"}` |
...
- HTTP status: `400`.
- Response body/headers/assertions: Không reflect executable HTML/script; không 5xx..
...
| Data row key | `TC-FR03-API-SEC-003` |
...
| Source | `AI` |
| Audit label | `VALID` |
| Human reasoning | Input không được reflect/leak nguy hiểm. |
