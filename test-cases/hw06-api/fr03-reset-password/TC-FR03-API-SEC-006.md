# TC-FR03-API-SEC-006: Reuse resetToken sau reset thành công bị reject
...
- Đã dùng token một lần thành công.
...
| Authorization | `none` |
| Request body/query/path | `{"email":"{{fr03Email}}","resetToken":"{{usedResetToken}}","newPassword":"Another123!"}` |
...
| Data row key | `TC-FR03-API-SEC-006` |
...
| Source | `AI` |
| Audit label | `VALID` |
| Human reasoning | OTP phải one-time use. |
