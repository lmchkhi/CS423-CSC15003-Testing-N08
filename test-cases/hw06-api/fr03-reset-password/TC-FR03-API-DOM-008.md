# TC-FR03-API-DOM-008: `email` có whitespace đầu/cuối
...
- Có valid resetToken từ setup cho `test@eshop.com`.
...
| Authorization | `none` |
| Request body/query/path | `{"email":" test@eshop.com ","resetToken":"{{validResetToken}}","newPassword":"NewPass123!"}` |
...
- HTTP status: `4xx`.
- Response body/headers/assertions: Không reset thành công cho email có whitespace; không 5xx..
...
| Data row key | `TC-FR03-API-DOM-008` |
...
| Human reasoning | Whitespace trim không được spec mô tả; giữ làm boundary và assert không được bypass token binding. |
