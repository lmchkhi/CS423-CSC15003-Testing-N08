# TC-FR03-API-SEC-008: Forgot-password setup trả OTP 6 digits trước khi reset
...
| Authorization | `none` |
| Request body/query/path | `setup `POST /api/forgot-password`, sau đó reset với token trả về` |
...
- HTTP status: `200 setup, 200 reset`.
- Response body/headers/assertions: Setup response có `resetToken` match `^[0-9]{6}$`; reset success có `message`..
...
| Data row key | `TC-FR03-API-SEC-008` |
...
| Source | `AI` |
| Audit label | `VALID` |
| Human reasoning | OTP format observable qua public setup API. |
...
Related bug report: `reports/bug-reports/BUG-FR03-API-002.md`
