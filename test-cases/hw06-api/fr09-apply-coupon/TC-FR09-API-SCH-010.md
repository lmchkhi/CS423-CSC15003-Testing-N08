# TC-FR09-API-SCH-010: Success response không expose internal coupon config fields

## Requirement ID
FR-09

## API / Test type / Technique
`POST /api/apply-coupon` / API Testing / `Schema Validation`
...
- HTTP status: `200`.
- Response body/headers/assertions: Không có id, type, discount_value, min_order_amount, expired_at, max_uses_per_user, is_active.
- Không có unexpected `5xx` nếu đây là negative/security case.
- Rationale: API spec chỉ cần discount_amount và final_amount cho checkout calculation.
...
| Data row key | `TC-FR09-API-SCH-010` |
...
| Human reasoning | AI kiểm tra không leak token/password nhưng bỏ sót internal coupon configuration leak. |
| Why AI missed it | AI kiểm tra không leak token/password nhưng bỏ sót internal coupon configuration leak. |

## Status / Related bugs

`Failed`

Related bug report: `reports/bug-reports/BUG-FR09-API-008.md`
