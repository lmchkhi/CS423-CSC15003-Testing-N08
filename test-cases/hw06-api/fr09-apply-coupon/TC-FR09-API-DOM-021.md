# TC-FR09-API-DOM-021: Valid coupon code có whitespace đầu/cuối
...
- User logged in; SAVE10 chưa dùng bởi user này.
...
| Request body/query/path | `{"code":" SAVE10 ","total_amount":500000,"user_id":{{userId}}}` |
...
- HTTP status: `4xx`.
- Response body/headers/assertions: message hoặc error; không có discount_amount/final_amount success.
- Không có unexpected `5xx` nếu đây là negative/security case.
- Rationale: Kiểm tra trim/matching ambiguity cho mã hợp lệ có whitespace.
...
| Data row key | `TC-FR09-API-DOM-021` |
...
| Human reasoning | AI có whitespace-only và lowercase nhưng bỏ sót valid code kèm whitespace đầu/cuối. |
| Why AI missed it | AI có whitespace-only và lowercase nhưng bỏ sót valid code kèm whitespace đầu/cuối. |
