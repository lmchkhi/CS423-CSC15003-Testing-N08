# TC-FR09-API-WF-008: IDOR rejection không consume quota của real owner
...
- Login user A; gửi body user_id user B; sau đó login B.
...
| Request body/query/path | `Mismatched request, then valid B request` |
...
- Response body/headers/assertions: User B vẫn apply coupon được.
- Không có unexpected `5xx` nếu đây là negative/security case.
- Rationale: Raw exact status/setup chưa rõ; sửa thành 4xx rồi verify quota owner ở bước hai.
...
| Data row key | `TC-FR09-API-WF-008` |
...
| Human reasoning | Giữ từ raw AI sau human audit; expected result được chuẩn hóa theo FR-09/API spec khi raw case còn mơ hồ. |
...
Related bug report: `reports/bug-reports/BUG-FR09-API-001.md`, `reports/bug-reports/BUG-FR09-API-004.md`
