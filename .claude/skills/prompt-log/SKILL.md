...
Fine to batch a few consecutive entries into one commit if they happened in
the same short burst of work. §12 wants a commit per step of the procedure
(each test plan, each run, the AI analysis, the proposal) — regenerating the
raw prompt trail is not one of those steps, so it does not need its own.
