...
- Name: shopping-cart.spec.mjs >> FR-07 - Giỏ hàng >> TC-CART-011: Hiển thị nút trừ để giảm số lượng
...
Locator: getByRole('row').filter({ hasText: 'Bàn phím cơ FR07' }).getByRole('button', { name: '-', exact: true })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('row').filter({ hasText: 'Bàn phím cơ FR07' }).getByRole('button', { name: '-', exact: true })
