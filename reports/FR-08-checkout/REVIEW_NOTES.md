...
> Đã đọc 18/18 case và chạy 15 điểm kiểm tra độc lập. Dữ liệu thật dùng AirPods Pro 2 (`id=4`, `6,000,000₫ × 2 = 12,000,000₫`). Case stateful được cô lập bằng user tạm role `user`. `Khớp/Lệch` dưới đây so với expected HW02 đã được chốt theo quyết định người dùng; `DT-012` chưa đủ bằng chứng UI nên giữ `Không xác định`.
...
| `TC-FR08-DT-012` | XSS phải hiển thị an toàn | API lưu nguyên chuỗi; chưa điều hướng được tới Orders view để kiểm tra render | Không xác định | Giữ expected hiển thị an toàn; cần xác minh UI sau | `evidence/phase-a-api-results.md`, `evidence/phase-a-ui-results.md` |
...
- Trong 15 điểm độc lập: `3 Khớp`, `11 Lệch`, `1 Không xác định` (`DT-012` phần UI).
...
| `DT-012` phần render UI | FR-08 UI không có điểm nhập/hiển thị địa chỉ; API-only đã duyệt | Đã thử Admin UI ở Phase A nhưng không tới Orders view | Không kiểm chứng sink XSS UI trong FR-08 | Chuyển assertion render sang FR-18; không tuyên bố Pass UI |
...
| Chromium | not run | Chưa đến Giai đoạn D |
| Firefox | not run | Chưa đến Giai đoạn D |
| Edge | Chỉ quan sát checkpoint A; chưa chạy suite đa trình duyệt | `evidence/phase-a-ui-results.md` |
| Metadata report | Chưa thấy | Chưa có report runtime |
...
- Điểm chưa chắc chắn: `DT-012` chưa xác minh được Orders UI.
