...
def main() -> int:
    ...
    stage = pathlib.Path(tempfile.mkdtemp(prefix="hw04-pdf-"))
    jobs = []
    ...
    # `playwright` resolves out of automation/node_modules, so run there.
    result = subprocess.run(
        ["node", str(REPO / "reports" / "tools" / "html-to-pdf.mjs"),
         str(jobs_file)],
        cwd=REPO / "automation",
    )
