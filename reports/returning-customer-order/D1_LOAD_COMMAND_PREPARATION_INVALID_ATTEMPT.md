# D1 Load — Chuẩn bị lệnh cho User thực thi

> **KHÔNG TÁI SỬ DỤNG:** run `20260813-233819-user-executed` đã chạy 0 sample và được phân loại
> `INVALID` do JMX thiếu `ThreadGroup.main_controller`. Dùng
> `D1_LOAD_COMMAND_PREPARATION_RERUN1.md` và run folder mới sau correction.
...
- [x] JMX XML parse thành công; SHA-256 `9C9D295F11C5F2315B74E7BCB47201AB8D388DCB063EEEF44C16E94AE33038C8`.
...
- [x] Precheck lúc `13/08/2026 23:36–23:39 +07:00`: `GET http://127.0.0.1:3000/api/products` trả HTTP 200; listener port 3000 thuộc PID 25832, process `node.exe`.
- [ ] PID 25832 chỉ là quan sát trước reset. User bắt buộc restart backend và resolve PID mới; không được dùng PID cũ làm evidence measured run.
...
- Created: `13/08/2026 23:38:19 +07:00`
- Raw JTL: `result.jtl`
...
$runDir = "E:\Testing\CS423-CSC15003-Testing-N08\tests\returning-customer-order\test-runs\load\20260813-233819-user-executed"
...
$expectedJmxHash = "9C9D295F11C5F2315B74E7BCB47201AB8D388DCB063EEEF44C16E94AE33038C8"
...
$jmeter = "D:\apache-jmeter-5.6.3\bin\jmeter.bat"
$jmx = "E:\Testing\CS423-CSC15003-Testing-N08\tests\returning-customer-order\test-cases\load\23127464_Load_20260813.jmx"
$jtl = "E:\Testing\CS423-CSC15003-Testing-N08\tests\returning-customer-order\test-runs\load\20260813-233819-user-executed\result.jtl"
$jmeterLog = "E:\Testing\CS423-CSC15003-Testing-N08\tests\returning-customer-order\test-runs\load\20260813-233819-user-executed\jmeter.log"
$consoleLog = "E:\Testing\CS423-CSC15003-Testing-N08\tests\returning-customer-order\test-runs\load\20260813-233819-user-executed\jmeter-console.log"
$baseUrl = "http://127.0.0.1:3000"
...
$jmeter = "D:\apache-jmeter-5.6.3\bin\jmeter.bat"
$jtl = "E:\Testing\CS423-CSC15003-Testing-N08\tests\returning-customer-order\test-runs\load\20260813-233819-user-executed\result.jtl"
$htmlDir = "E:\Testing\CS423-CSC15003-Testing-N08\tests\returning-customer-order\test-runs\load\20260813-233819-user-executed\html-report"
$htmlGenerationLog = "E:\Testing\CS423-CSC15003-Testing-N08\tests\returning-customer-order\test-runs\load\20260813-233819-user-executed\html-generation.log"
...
- Nếu thất bại: dừng trước measured run. Không sửa hash/JMX/CSV để lách precheck; không dùng PID 25832 cũ nếu backend đã restart.
...
powershell.exe -NoProfile -ExecutionPolicy Bypass `
    ...
    -EvidencePath "E:\Testing\CS423-CSC15003-Testing-N08\tests\returning-customer-order\test-runs\load\20260813-233819-user-executed\account-provisioning.txt" `
    -BaseUrl "http://127.0.0.1:3000" `
    -Scenario "Load" `
    -ExpectedCount 20
...
$monitor = Start-Process powershell.exe `
    -ArgumentList @(
        ...
        "-BackendPid", $backendPid,
        "-OutputPath", "E:\Testing\CS423-CSC15003-Testing-N08\tests\returning-customer-order\test-runs\load\20260813-233819-user-executed\backend-resource.csv",
        "-StopFile", "E:\Testing\CS423-CSC15003-Testing-N08\tests\returning-customer-order\test-runs\load\20260813-233819-user-executed\monitor.stop",
        "-IntervalSeconds", "2"
    ) `
    -WindowStyle Hidden `
    -PassThru
...
Start-Sleep -Seconds 3
if ($monitor.HasExited) { throw "Resource monitor exited before workload start" }
if (-not (Test-Path -LiteralPath "E:\Testing\CS423-CSC15003-Testing-N08\tests\returning-customer-order\test-runs\load\20260813-233819-user-executed\backend-resource.csv")) {
...
- Đọc rõ: “Tôi là sinh viên 23127464. Đây là D1 Load, Executor: User. JMX là 23127464_Load_20260813.jmx. Run folder là 20260813-233819-user-executed. Base URL là http://127.0.0.1:3000. Workload gồm 20 VU, ramp-up 60 giây, steady state 360 giây, think time ngẫu nhiên 1 đến 3 giây, Summary Report. Backend PID hiện tại là [đọc số thật trong backend-pid.txt].”
...
```powershell
Get-Content -LiteralPath "E:\Testing\CS423-CSC15003-Testing-N08\tests\returning-customer-order\test-runs\load\20260813-233819-user-executed\jmeter-console.log" | Select-Object -Last 3
```
...
```powershell
$stopFile = "E:\Testing\CS423-CSC15003-Testing-N08\tests\returning-customer-order\test-runs\load\20260813-233819-user-executed\monitor.stop"
$resourceOutput = "E:\Testing\CS423-CSC15003-Testing-N08\tests\returning-customer-order\test-runs\load\20260813-233819-user-executed\backend-resource.csv"
...
```powershell
notepad.exe "E:\Testing\CS423-CSC15003-Testing-N08\tests\returning-customer-order\test-runs\load\20260813-233819-user-executed\user-execution-note.md"
```
...
- PID trước reset là 25832 và không được dùng sau restart; PID measured phải lấy từ `backend-pid.txt` mới.
...
`D1 LOAD COMMAND READY — PENDING USER EXECUTION`
