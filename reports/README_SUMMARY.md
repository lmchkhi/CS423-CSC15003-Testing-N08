...
| Thời điểm kiểm kê | `09/08/2026 19:23` |
...
| **Tổng artifact hiện có** | **76** | **73** | **219** | **93** | **126** | **0** | **9** | **18** |

Số liệu trên chỉ cộng từ các HTML report/evidence đã có. FR-08 có 18 case HW02 vật lý, trong đó ba case trùng được gộp thành 15 điểm API độc lập, cộng thêm 6 case UI bổ sung. Ba UI case mới tăng coverage nhưng ánh xạ root cause/report hiện có nên tổng bug vẫn là 18. Các GitHub Issue hiện hành: FR-05 `#220`–`#225`, FR-08 `#226`–`#233`, FR-12 `#234`–`#236` và `#238`; screenshot Issue đã được lưu và chèn vào từng report.
...
| Failure classification | 8 case / 6 SUT root causes | 17 case / 8 SUT root causes | 17 case / 4 SUT root causes | Đủ; test/environment issue được tách riêng |
...
| `FR-08` | `4P/17F/0S` | `4P/17F/0S` | `4P/17F/0S` | `playwrite-test/FR-08-checkout/playwright-report/index.html` |
| `FR-12` | `23P/17F/0S` | `23P/17F/0S` | `23P/17F/0S` | `playwrite-test/fr12-access/playwright-report/index.html` |

FR-08 hiện gồm 15 case API và 6 case UI thật; 18 lượt UI dùng browser page/DOM trên ba project, còn DT-012 vẫn API-only. FR-12 dùng hybrid Web Admin UI + API: cả 40 case đều mở page thật để kiểm tra cổng truy cập, sau đó kiểm chứng sâu enforcement backend. FR-05 dùng UI làm oracle chính cho cả 12 case.
