...
| **Mã bài tập :**        | HW02                    |
| **Ngày làm bài:**       | 23-06-2026 - 28/06/2026 |
...
````-001** | BUG-001 + BUG-005 | Thiếu trường "Xác nhận mật khẩu" → block TC-001 và TC-013 |
| **BUG-FR-01-002** | BUG-002 + BUG-003 + BUG-004 | Hệ thống ưu tiên validate MK trước email, báo lỗi MK yếu thay vì lỗi email |
| **BUG-FR-01-003** | BUG-006 + BUG-007 | Logic validate MK sai — MK hợp lệ (8-9 ký tự đủ điều kiện) bị từ chối |

### Files đã xóa:
- `BUG-FR-01-004.md`, `BUG-FR-01-005.md`, `BUG-FR-01-006.md`, `BUG-FR-01-007.md`

### Test Run:
- Cập nhật tất cả status và bug reference cho nhất quán.
- Tổng kết mới: 8 Passed, 4 Failed, 2 Blocked, 0 Not Run.
`````
...
- AI vẫn giữ status của một số test case như TC-FR-01-008, TC-FR-01-009,... là Passed/Failed thay vì đổi thành Blocked. Vì các test case này đều có step nhập mật khẩu và xác nhận mật khẩu nên không thể test.

### (5) Student Fix

- Sửa lại status và ghi chú related bug cho các test case còn sót l
...
| ----------------- | ------------------------------ |
| **Tool**          | GPT-5.5                        |
| **Timestamp**     | 01:50 PM 26/06/2026            |
| **Artifact type** | Bug report cho `BUG-FR-24-001` |

**Full prompt:**

```
Với các test case bị ảnh hưởng cụ thể bởi việc thiếu trường xác nhận mật khẩu trong FR-24, hãy giúp tôi đánh Failed/Blocked cho phù hợp rồi dùng skill @skills/generate-bug-report/skill.md để tạo ra bug cho việc thiếu trường đó
```

### (2) AI Output

```
Mình đã xác định đúng các test case liên quan nhất: `TC-FR-24-001` và `TC-FR-24-013` là bị ảnh hưởng trực tiếp, còn các case khác không bị block bởi lỗi này nếu vẫn có thể test các field còn lại. Giờ mình sẽ cập nhật status và tạo bug report theo skill.
Mình đã tạo `BUG-FR-24-001`; tiếp theo cập nhật test run để summary khớp với status mới.
Đã cập nhật đúng theo `FR-24`:

- `TC-FR-24-001` -> `Blocked / BUG-FR-24-001`
- `TC-FR-24-013` -> `Failed / BUG-FR-24-001`
- Tạo bug report mới: `bug-reports/BUG-FR-24-001.md`
- Cập nhật test run: `tests/test-runs/FR-24-register-run.md`

Lý do:
- `TC-FR-24-001` không thực thi đúng kịch bản vì thiếu field `Xác nhận mật khẩu`, nên nên `Blocked`.
- `TC-FR-24-013` kiểm trực tiếp requirement “xác nhận mật khẩu không khớp thì bị từ chối”, nên `Failed` vì app không hỗ trợ đúng requirement.

Nếu muốn, mình có thể tiếp tục rà toàn bộ `FR-24` để xem test case nào khác cũng cần đổi từ `Not Run` sang `Blocked` do cùng lỗi này.
```
...
- AI tạo ra bug report đúng yêu cầu nhưng chưa thực hiện việc đánh Failed/Blocked cho đầy đủ các test case bị ảnh hưởng bởi bug này.

### (5) Student Fix

- Trình bày lại để AI tiếp tục đánh blocked các test case bị ảnh hưởng khác.
...
| **Tool**          | GPT-5.5                                |
| **Timestamp**     | 01:50 PM 26/06/2026                    |
| **Artifact type** | Update test cases / test run for FR-24 |

**Full prompt:**

```
Tại sao các test case khác cũng có test step nhận xác nhận mật khẩu nhưng chưa bị báo Blocked
```

### (2) AI Output

```
Bạn đúng: nếu chấm theo test case đã viết, mọi case có step bắt buộc nhập `Xác nhận mật khẩu` đều bị ảnh hưởng. Mình đã xử lý hơi hẹp theo “mục tiêu kiểm thử trực tiếp”; giờ mình sẽ chỉnh các case còn lại sang `Blocked` để nhất quán với test steps.
Bạn bắt đúng lỗi trong cách xử lý trước của mình.

Các test case khác cũng có step nhập `Xác nhận mật khẩu`, nên nếu giữ nguyên test case như đã viết thì chúng **cũng bị Blocked**, vì tester không thể thực hiện đầy đủ test steps trên app thật.

Mình đã chỉnh lại cho nhất quán:

- `TC-FR-24-001` -> `Blocked`
- `TC-FR-24-002` đến `TC-FR-24-012` -> `Blocked`
- `TC-FR-24-013` -> `Failed`
- `TC-FR-24-014` đến `TC-FR-24-016` -> `Blocked`

Tổng kết mới trong `tests/test-runs/FR-24-register-run.md`:
...
Lý do giữ `TC-FR-24-013` là `Failed`: test case này kiểm trực tiếp requirement “xác nhận mật khẩu không khớp thì bị từ chối”, nhưng app không có field đó nên requirement này không được hỗ trợ.

Các file đã cập nhật:

- `tests/test-runs/FR-24-register-run.md`
- `tests/test-cases/FR-24-register(mobile)/TC-FR-24-002.md` đến `TC-FR-24-012.md`
- `tests/test-cases/FR-24-register(mobile)/TC-FR-24-014.md` đến `TC-FR-24-016.md`

Bug dùng chung vẫn là:

- `bug-reports/BUG-FR-24-001.md`
```
...
- AI thành công đánh lại failed/blocked cho tất cả các test case bị ảnh hưởng bởi bug này.
...
| ----------------- | ------------------------------------------------------------------------------------- |
| **Tool**          | Claude Opus 4.6                                                                       |
| **Timestamp**     | 03:09 PM 26/06/2026                                                                   |
| **Artifact type** | Đánh giá lại status test cases FR-01 (Failed vs Blocked) và gộp bug reports duplicate |

**Full prompt:**

```
Giúp tôi xem lại các test case và bug report của FR-01 xem có cần sửa lại cho phù hợp hay merge một số bug lại không. Vì website không có trường nhập lại mật khẩu (có trong test data và test step của đa số test case). Nếu có hãy giúp tôi sửa và cập nhật lại các test case, bug report, và test run của FR-01 cho phù hợp
```

### (2) AI Output

```
Searched for all FR-01 related files in the workspace:
- tests/test-cases/FR-01-register/ (16 files: TC-FR-01-001.md to TC-FR-01-016.md)
- bug-reports/ (7 files: BUG-FR-01-001.md to BUG-FR-01-007.md)
- tests/test-runs/FR-01-register-run.md

Viewed all 16 test case files, all 7 bug report files, and the test run file.

Asked user: TC-FR-01-013 should be deleted or marked N/A?
User answered: "Không xóa test case nào hết, nhưng xem lại liệu chúng là failed hay là blocked, do trường xác nhận mật khẩu không có nằm trong test steps"

Asked user: Confirm approach — TC-013 = Blocked, others keep status?
User answered: "Giữ hết test case lẫn test data và test step, chỉ đánh giá lại kết quả là failed hay blocked thôi. Và các bug có bị duplicate hay không, có cần gộp lại hay k"

Analysis completed:
- TC-FR-01-001: Failed → Blocked (test step yêu cầu nhập trường không tồn tại)
- TC-FR-01-013: Failed → Blocked (tương tự)
- TC-FR-01-002, 003, 007-012, 015: Not Run → Passed (đã thực thi và pass)
- TC-FR-01-004, 005, 006, 014, 016: giữ Failed

Bug duplicate analysis:
- BUG-002 + BUG-003 + BUG-004: cùng root cause (validate MK trước email)
- BUG-006 + BUG-007: cùng root cause (logic validate MK sai)
- BUG-001 + BUG-005: cùng vấn đề thiếu trường xác nhận MK

Asked user: Confirm merge approach (3 bugs)?
User answered: "Gộp theo đề xuất (3 bug)"

Edited TC-FR-01-001.md: Status → Blocked / BUG-FR-01-001
Edited TC-FR-01-013.md: Status → Blocked / BUG-FR-01-001
Wrote BUG-FR-01-001.md (merged BUG-001 + BUG-005): Thiếu trường "Xác nhận mật khẩu"
Wrote BUG-FR-01-002.md (merged BUG-002 + BUG-003 + BUG-004): Hệ thống báo lỗi MK yếu thay vì lỗi Email
Wrote BUG-FR-01-003.md (merged BUG-006 + BUG-007): MK hợp lệ bị từ chối
Deleted BUG-FR-01-004.md, BUG-FR-01-005.md, BUG-FR-01-006.md, BUG-FR-01-007.md
Edited TC-FR-01-004.md: Status → Failed / BUG-FR-01-002, BUG-FR-01-003
Edited TC-FR-01-005.md: Status → Failed / BUG-FR-01-002, BUG-FR-01-003
Edited TC-FR-01-006.md: Status → Failed / BUG-FR-01-002, BUG-FR-01-003
Edited TC-FR-01-014.md: Status → Failed / BUG-FR-01-003
Edited TC-FR-01-016.md: Status → Failed / BUG-FR-01-003
Wrote tests/test-runs/FR-01-register-run.md (updated all status + summary)

Verified final consistency with grep across all TC files.

---

## Tóm tắt thay đổi FR-01

### Test Cases — Thay đổi status:
| TC | Trước | Sau | Lý do |
|---|---|---|---|
| TC-FR-01-001 | Failed | **Blocked** | Test step yêu cầu nhập trường "Xác nhận mật khẩu" nhưng trường không tồn tại |
| TC-FR-01-013 | Failed | **Blocked** | Tương tự — không thể test luồng xác nhận MK không khớp |
| TC-FR-01-002, 003, 007-012, 015 | Not Run | **Passed** | Đã thực thi và pass (file TC riêng lẻ đã đúng từ trước, test run cập nhật lại) |
| TC-FR-01-004, 005, 006, 014, 016 | Failed | **Failed** (giữ nguyên) | Lỗi thật — hệ thống báo sai thông báo lỗi |

### Bug Reports — Gộp từ 7 → 3:

| Bug mới | Gộp từ | Mô tả |
|---|---|---|
| **BUG-FR-01-001** | BUG-001 + BUG-005 | Thiếu trường "Xác nhận mật khẩu" → block TC-001 và TC-013 |
| **BUG-FR-01-002** | BUG-002 + BUG-003 + BUG-004 | Hệ thống ưu tiên validate MK trước email, báo lỗi MK yếu thay vì lỗi email |
| **BUG-FR-01-003** | BUG-006 + BUG-007 | Logic validate MK sai — MK hợp lệ (8-9 ký tự đủ điều kiện) bị từ chối |
...
- Sửa lại status và ghi chú related bug cho các test case còn sót lại. Rút gọn chỉ còn 1 bug vì bug đó block hết các test case.
...
| ----------------- | -------------------------------------------- |
| **Tool**          | Codex (GPT-5)                                |
| **Timestamp**     | 11:07 PM 28/06/2026                          |
| **Artifact type** | Cập nhật README và tổng hợp kết quả kiểm thử |

**Full prompt:**

```
Dựa vào các docs và reports trong repo của tôi, hãy giúp tôi điền các thông tin đúng vào file này thay cho placeholder, riêng self-assessed grade để trống cho tôi tự điền
```

### (2) AI Output

```
Mình sẽ đối chiếu README với toàn bộ docs/reports trong repo, xác định từng placeholder và điền bằng thông tin có bằng chứng; mục self-assessed grade sẽ được giữ trống đúng yêu cầu.

Searched for placeholders and incomplete fields in README, reports, docs, skills, and git-log.txt.
Viewed README.md and listed all repository files.

README hiện đang lệch khá xa trạng thái repo: thực tế có đủ 4 feature (FR-01, FR-07, FR-16, FR-24), 64 test case và nhiều bug report. Mình đang lấy số liệu trực tiếp từ các test-run/bug-report thay vì dựa vào bảng mẫu cũ để tránh cộng nhầm.

Viewed reports/main-report.md and all files in tests/test-runs/.
Searched the repository for student information, video links, issue numbers, severity, priority, and status data.
Inspected the Git remote, commit authors, working-tree status, and student ID references.

Mình đã tìm được MSSV `23127062`, tên `Lâm Vĩ Khang`, cùng GitHub Issue #8 và #18–#33 từ audit/bug reports. Phần video không có URL nào trong repo, nên mình sẽ ghi rõ “Chưa cung cấp” thay vì giữ link giả; còn cột tự chấm (kể cả ô tổng) sẽ để trống hoàn toàn.

Attempted to verify GitHub issue pages and states.
Extracted severity, priority, and issue numbers from all bug reports.
Updated README.md with student information, all four features, test summaries, bug summaries, issue links, and demo-video availability.

Phần chính đã được điền xong. Mình đang kiểm tra chéo lần cuối các tổng: `64 = 6 Passed + 19 Failed + 39 Blocked`, và rà README để chắc không còn `TBD`, `FR-xx`, link giả hay dữ liệu tự chấm bị điền nhầm.

Checked the README diff, Markdown whitespace, remaining placeholders, and test-run statistics.

Đã cập nhật [README.md](/home/lvkhang/Documents/school/software-testing/CS423-CSC15003-Testing-N08-report/README.md):

- Điền thông tin sinh viên và 4 feature.
- Tổng hợp chính xác 64 test case và 17 bug.
- Thêm liên kết GitHub Issue.
- Đánh dấu video chưa được cung cấp.
- Giữ trống toàn bộ cột Self-Assessed Grade.
- Đã kiểm tra không còn placeholder hoặc lỗi Markdown.
```
...
- AI đã điền đầy đủ thông tin vào README dựa trên các artifact trong repo, giữ trống phần self-assessed grade theo yêu cầu.
...
| ----------------- | ----------------------------------------------------- |
| **Tool**          | Codex (GPT-5.5)                                       |
| **Timestamp**     | 11:15 PM 28/06/2026                                   |
| **Artifact type** | Đổi mã feature mobile từ FR-24 thành FR-25 trong repo |

**Full prompt:**

```
Cập nhật tất cả thông tin về FR-24 thành FR-25, trừ trong file ai audit report
```

### (2) AI Output

```
Mình sẽ đổi toàn bộ mã feature `FR-24` sang `FR-25` trong nội dung, tên thư mục và tên file liên quan, đồng thời loại trừ hoàn toàn `reports/ai-audit-report.md` như bạn yêu cầu.

Searched for all `FR-24` and `fr-24` references outside reports/ai-audit-report.md and listed affected paths.
Inspected FR-24 references in generated PDFs and git-log.txt.

Phạm vi đổi gồm 16 test case, test-run, bug report, README và main report; mình cũng sẽ đổi tên các file/thư mục để mã trong đường dẫn không còn là `FR-24`. `ai-audit-report.md` và bản PDF audit sẽ được giữ nguyên, kể cả các tham chiếu lịch sử bên trong đó.

Replaced FR-24 with FR-25 in README.md, reports/main-report.md, the test run, the bug report, and all 16 test cases.
Renamed the FR-24 test-case directory, all test-case files, the test-run file, and the bug-report file to FR-25.
Inspected the existing PDF generation metadata and available PDF tools.

Mình phát hiện `pdfs/main-report.pdf` cũng chứa FR-24. Vì đây là bản xuất từ `main-report.md`, mình đang đồng bộ lại PDF này; bản audit PDF vẫn giữ nguyên theo ngoại lệ của bạn.

Generated and visually inspected an updated A4 main-report PDF.
Copied the regenerated PDF to pdfs/main-report.pdf.
Verified that no FR-24 references or filenames remained outside the excluded audit artifacts.

Đã đổi xong mã và toàn bộ đường dẫn liên quan. Bước kiểm tra cuối xác nhận không còn `FR-24` ngoài hai artifact audit được loại trừ; bộ FR-25 vẫn đủ 16 test case tuần tự, một test-run và một bug report.

Verified all 16 FR-25 test cases, the FR-25 test run, the FR-25 bug report, and that both audit artifacts were unchanged.

Đã cập nhật toàn bộ `FR-24` thành `FR-25`, gồm:

- README và main report.
- 16 test case cùng thư mục.
- Test-run và bug report.
- Bản PDF main report.
- Tất cả mã `TC-FR-25-*` và `BUG-FR-25-*`.

[ai-audit-report.md](/home/lvkhang/Documents/school/software-testing/CS423-CSC15003-Testing-N08-report/reports/ai-audit-report.md) và PDF audit được giữ nguyên. Không còn tham chiếu `FR-24` nào ngoài hai file audit.
```
...
- AI đã hoàn tất việc đổi mã feature từ FR-24 sang FR-25 trong toàn bộ repo, ngoại trừ các artifact audit như yêu cầu.
