...
| 3 | 1:10–2:00 | Mở `diagrams/test-generator.png` (bản tự vẽ) + `reports/test-generator-design.md` | "Đây là sơ đồ thiết kế em **tự vẽ** theo §11: parse spec → phân vùng từng tham số → bộ luật security SEC-01 đến 07 → trích schema oracle → mô hình chuyển trạng thái → bộ phát sinh case → ghi data file. Phần pseudocode nằm ngay trong báo cáo thiết kế." |
| 4 | 2:00–3:20 | Terminal: gọi skill sinh case cho `POST /api/categories` vào `demo-scratch/` | "Bây giờ em cho skill sinh test cho endpoint tạo danh mục. Nó đi lần lượt: phân vùng trường `name` — hợp lệ, rỗng, thiếu, rất dài, unicode, trùng, SQL-meta, XSS; rồi access-control — admin, user thường, không token; rồi lifecycle CRUD; rồi contract khi id không tồn tại. Kết quả là bảng `TC-FR14-*` và data file demo trong `demo-scratch/`." |
...
| 7 | 4:40–5:40 | Terminal: `bash api/scripts/boot-sut.sh && bash api/scripts/run-fr14.sh` — để log chạy | "Chạy suite thật bằng Newman trên localhost:3000. Chú ý log in ra dòng `[HW06] X-Student-Id = 23127300` cho **mọi** request — đây là header chống gian lận §11, gắn tự động qua pre-request script cấp collection. Các case known-bug được gắn nhãn `[BUG-*]` và kiểm hành vi quan sát được." |
| 8 | 5:40–6:20 | Mở report HTML htmlextra vừa sinh, rồi tab GitHub Issues (#258–#261) | "Report htmlextra tổng hợp pass/fail. Mỗi lỗi quan sát được em mở một GitHub Issue black-box — chỉ mô tả hành vi quan sát, curl tái hiện, kỳ vọng vs thực tế; không tham chiếu mã nguồn SUT." |
