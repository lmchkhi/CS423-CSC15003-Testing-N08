<!-- tests/test-cases/FR-10/decision-table/TC-FR10-DTT-009.md -->

# TC-FR10-DTT-009: Từ chối chuyển trạng thái từ canceled — trạng thái kết thúc (Decision Table Testing)
...
| ----------------------- | -------------------------------------------------- |
| Rule ID                 | R9                                                 |
| C1: Trạng thái hiện tại | `canceled`                                         |
...
```
canceled ──[Admin thay đổi]──► confirmed ❌ (Trạng thái kết thúc)
```
...
- Có ít nhất một đơn hàng đang ở trạng thái `canceled`
- Ghi nhận Order ID của đơn hàng canceled để sử dụng trong test
...
| Order ID       | (ID đơn hàng đang canceled)        |
...
3. Tìm đơn hàng có trạng thái "canceled" trong danh sách
...
7. Kiểm tra lại trạng thái đơn hàng — phải vẫn giữ nguyên `canceled`

## Expected result

- ❌ Hệ thống từ chối chuyển đổi trạng thái
- API trả về HTTP 400 với thông báo lỗi phù hợp (ví dụ: "Không thể thay đổi trạng thái đơn hàng đã hủy")
- Đơn hàng vẫn giữ nguyên trạng thái `canceled` — không bị thay đổi
- Trên giao diện Admin: không nên hiển thị các tùy chọn chuyển trạng thái cho đơn hàng canceled
