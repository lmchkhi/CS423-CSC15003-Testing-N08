<!-- tests/test-cases/FR-09-coupon/state-transition/TC-FR09-STT-008.md -->
# TC-FR09-STT-008: Remove coupon (State Transition Testing)
...
| --- | --- | --- | --- | --- | --- |
| TC-FR09-STT-008 | S2: Applied | E2: Remove Coupon | — | S0: Idle | ✅ Gỡ coupon, tổng tiền trở về giá gốc |
...
- S2: Applied (Mã SAVE10 đang được áp dụng)
...
> Mã đang được áp dụng, đang có discount

## Test steps

1. Ở trạng thái S2 (Applied), nhìn thấy tổng tiền đã giảm
2. Bấm nút 'Xóa mã' hoặc 'Hủy mã' (E2)
3. Quan sát giao diện

## Expected result

Chuyển về S0: Idle. Ô nhập mã bị xóa trống (hoặc hiện lại nút Apply). Tổng tiền trở về mức giá gốc (total_amount) ban đầu.

## Actual result

Không có luồng xóa coupon đúng nghĩa trong Checkout UI; mã đã áp dụng không có nút remove để đưa về S0.
