<!-- tests/test-cases/FR-09-coupon/state-transition/TC-FR09-STT-003.md -->

# TC-FR09-STT-003: C1=F Rejected (NOTEXIST) (State Transition Testing)
...
| --------------- | -------------- | ------ | ----------------------- | ------------ | ---------------------------------------------- |
| TC-FR09-STT-003 | S1: Validating | (auto) | C1=F (mã không tồn tại) | S3: Rejected | ✅ ❌ Thông báo lỗi 'Mã giảm giá không hợp lệ' |

> **Ghi chú:** Valid transition (Error path), isolates C1=False
...
> Mã NOTEXIST không có trong CSDL

## Test steps

1. Tại trang Checkout, nhập mã NOTEXIST
2. Bấm 'Áp dụng' để hệ thống Validating (S1)
3. Đợi hệ thống validate xong

## Expected result

Hệ thống chuyển sang S3: Rejected. Hiển thị thông báo lỗi 'Mã giảm giá không hợp lệ'. Tổng tiền giữ nguyên.
