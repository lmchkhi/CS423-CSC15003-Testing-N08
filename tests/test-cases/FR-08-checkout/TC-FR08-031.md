# TC-FR08-031: Bảo mật - IDOR đọc đơn hàng user khác
...
|---|---|---|---|---|
| TC-FR08-031 | GET /api/orders/:id với userB token | Đơn hàng thuộc về userA | 403 Forbidden (phải từ chối quyền truy cập) | Mã phản hồi 403 |
...
| Dữ liệu / Hành động | GET /api/orders/:id với userB token |
| Nhóm kiểm thử | Bảo mật - IDOR đọc đơn hàng user khác |
...
## Kết quả mong đợi
403 Forbidden (phải từ chối quyền truy cập)

Oracle: Mã phản hồi 403
