# TC-FR08-EXT-005: Ép kiểu - total_amount dạng chuỗi
...
|---|---|---|---|---|
| TC-FR08-EXT-005 | POST /api/checkout với total_amount="50000" (string thay vì number). | User token hợp lệ. | Quan sát được 200 OK hoặc bị ép kiểu; mong đợi schema nghiêm ngặt là 400 Bad Request. | AI có test chuỗi không phải số như `abc` nhưng bỏ sót chuỗi số; case này kiểm tra type validation yếu. |
...
## Kết quả mong đợi
Quan sát được 200 OK hoặc bị ép kiểu; mong đợi schema nghiêm ngặt là 400 Bad Request.

Oracle: AI có test chuỗi không phải số như `abc` nhưng bỏ sót chuỗi số; case này kiểm tra type validation yếu.
