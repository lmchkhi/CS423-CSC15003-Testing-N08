<!-- tests/test-cases/FR-08-checkout/domain-testing/TC-FR08-DT-007.md -->

# TC-FR08-DT-007: Hacker gửi total_amount quá cao — over-charge (Domain Testing)
...
| ------ | ---------------- | ---------------------------------------- | ----------------- | ---------------------- | -------------------------------------------------------------- |
| DT-007 | EP-V1-01 — Valid | EP-V2-05 — Cao hơn thực tế (99,999,999₫) | EP-V3-01 — Hợp lệ | EP-V4-01 — Có sản phẩm | ✅ Checkout thành công, tổng tiền = 200,000₫ (backend tự tính) |
...
| total_amount     | `99999999` _(hacker gửi giá trị rất cao so với thực tế)_ |
| shipping_address | `"123 Le Loi, Q1, TP.HCM"`                               |

> **Kịch bản tấn công:** Hacker gửi `total_amount = 99999999` qua Postman. Nếu backend tin client, người dùng có thể bị tính tiền quá cao so với giỏ hàng thực tế.
...
3. Gửi `POST /api/checkout` qua **Postman** với body `{"total_amount": 99999999, "shipping_address": "123 Le Loi, Q1, TP.HCM"}` và header `Authorization: Bearer <token>`
4. Kiểm tra response
5. Gửi `GET /api/orders/my-orders` để lấy đơn hàng mới nhất
6. **Kiểm tra `total_amount` trong đơn hàng** — giá trị phải là 200,000₫, KHÔNG phải 99,999,999₫

## Expected result

- Checkout thành công (HTTP 200 OK)
- Đơn hàng được tạo với `total_amount` = **200,000₫** (backend tự tính)
- Backend **bỏ qua** giá trị `total_amount = 99999999` do client gửi
- Nếu `total_amount` trong đơn hàng = 99,999,999₫ → 🐛 **BUG BẢO MẬT**: Người dùng bị tính phí quá cao

## Actual result

- Checkout thành công (HTTP 200 OK)
- Đơn hàng được tạo với `total_amount` = **99999999**
