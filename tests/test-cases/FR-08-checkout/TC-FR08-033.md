# TC-FR08-033: XSS trong shipping_address
...
|---|---|---|---|---|
| TC-FR08-033 | 200000 / "<script>alert('xss')</script>" / user | User token hợp lệ | 400 Bad Request hoặc sanitized | Mã phản hồi 400 hoặc stored safely |
...
| Dữ liệu / Hành động | 200000 / "<script>alert('xss')</script>" / user |
| Nhóm kiểm thử | XSS trong shipping_address |
...
## Kết quả mong đợi
400 Bad Request hoặc sanitized

Oracle: Mã phản hồi 400 hoặc stored safely
